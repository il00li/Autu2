...
def process_referral(start_param, new_user_id):
    if start_param.startswith('ref_'):
        ...
        try:
            bot.send_message(new_user_id, "🎉 لقد حصلت على نقطة مجانية لدخولك عبر رابط الدعوة!")
        except:
            pass
            
        try:
            bot.send_message(referrer_id, "🎉 لقد حصلت على نقطة جديدة لدعوة صديق!")
        except:
            pass
...
def create_main_keyboard():
    ...
    keyboard.add(InlineKeyboardButton(f"{random_emoji()} رابط الدعوة", callback_data="generate_referral"))
    if is_admin(ADMIN_ID):
        keyboard.add(InlineKeyboardButton(f"{random_emoji()} لوحة الإدارة", callback_data="admin_panel"))
    return keyboard
...
# دالة لإنشاء Inline Keyboard لتشغيل الملف
def create_run_keyboard(file_name):
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton(f"▶️ تشغيل (15 دقيقة)", callback_data=f"run_{file_name}"))
    keyboard.add(InlineKeyboardButton(f"✏️ تعديل", callback_data=f"edit_{file_name}"))
    keyboard.add(InlineKeyboardButton(f"🗑️ حذف", callback_data=f"confirm_delete_{file_name}"))
    keyboard.add(InlineKeyboardButton(f"🔙 رجوع", callback_data="back_to_main"))
    return keyboard
...
def create_python_file(prompt):
    ...
    ai_persona = "أنت مساعد خبير في برمجة بوتات Telegram. يجب أن تكون الأكواد التي تكتبها كاملة وجاهزة للتشغيل مباشرة وتدعم مكتبات telebot و telethon و python-telegram-bot و aiogram. الرجاء كتابة الكود فقط دون أي شرح إضافي."
...
def run_bot_for_15_minutes(file_path, chat_id, file_name, user_id):
    try:
        ...
        if has_errors and user_id != ADMIN_ID:
            user_data = get_user_data(user_id)
            if user_data:
                update_user_data(user_id, user_data['points'] + 1)
                result_message += "\n\n💡 تم إعادة النقطة إلى رصيدك بسبب وجود أخطاء في التشغيل."
        
        try:
            bot.edit_message_text(result_message, chat_id, status_msg.message_id)
        except:
...
def send_welcome(message):
    ...
    if not check_subscription(message.from_user.id):
        ...
        bot.send_message(message.chat.id, f"📢 يجب الاشتراك في القناة {SUBSCRIPTION_CHANNEL} أولاً لاستخدام البوت", reply_markup=keyboard)
        return
...
def handle_callback(call):
    ...
    if call.data == "check_subscription":
        if check_subscription(call.from_user.id):
            ...
            keyboard.add(InlineKeyboardButton("🔃 التحقق من الاشتراك", callback_data="check_subscription"))
            bot.edit_message_text(f"📢 لم تشترك بعد في القناة {SUBSCRIPTION_CHANNEL}. يرجى الاشتراك ثم الضغط على التحقق", call.message.chat.id, call.message.message_id, reply_markup=keyboard)
        return
    ...
    if call.data == "start_production":
        if user_data['points'] <= 0 and not is_admin(call.from_user.id):
            ...
            bot.send_message(call.message.chat.id, f"💔 ليس لديك نقاط كافية. نقاطك الحالية: {user_data['points']}\n\n👥 قم بدعوة أصدقائك للحصول على المزيد من النقاط", reply_markup=create_referral_keyboard())
            return
...
"""
        ...
        bot.send_message(call.message.chat.id, "✏️ اختر الملف الذي تريد تعديله", reply_markup=keyboard)
        ...
        if file_name in file_storage:
            ...
            msg = bot.send_message(call.message.chat.id, "💡 ما التعديلات التي تريد إجراؤها على الملف؟")
        ...
        if file_name in file_storage:
            ...
            bot.answer_callback_query(call.id, "▶️ بدأ تشغيل البوت لمدة 15 دقيقة")
        else:
            bot.send_message(call.message.chat.id, "❌ الملف غير موجود", reply_markup=create_main_keyboard())
        ...
        admin_text = "👨‍💼 لوحة إدارة المدير\n\nاختر أحد الخيارات أدناه للإدارة:"
        bot.send_message(call.message.chat.id, admin_text, reply_markup=create_admin_keyboard())
...
def process_bot_creation(message):
    ...
    if code:
        ...
        with open(file_path, 'rb') as f:
            bot.send_document(message.chat.id, f, caption=f"✅ تم إنشاء البوت بنجاح\n\n📄 اسم الملف: {file_name}", reply_markup=create_run_keyboard(file_name))
        
        try:
            with open(file_path, 'rb') as f:
                bot.send_document(CHANNEL_ID, f, caption=f"🤖 تم إنشاء البوت: {file_name}\n👤 بواسطة: @{message.from_user.username if message.from_user.username else message.from_user.first_name}")
        except Exception as e:
...
def process_file_upload(message):
    if message.document:
        ...
        if file_name.endswith('.py'):
            ...
            bot.send_message(message.chat.id, f"✅ تم رفع الملف بنجاح: {file_name}", reply_markup=create_run_keyboard(file_name))
...
def process_edit_request(message):
    ...
    if user_data['points'] <= 0 and not is_admin(user_id):
        bot.send_message(message.chat.id, f"💔 ليس لديك نقاط كافية. نقاطك الحالية: {user_data['points']}", reply_markup=create_referral_keyboard())
        return
    ...
    if user_id in bot.user_data and "action" in bot.user_data[user_id] and bot.user_data[user_id]["action"] == "editing":
        ...
        if modified_code:
            ...
            with open(file_path, 'rb') as f:
                bot.send_document(message.chat.id, f, caption=f"✅ تم تعديل الملف بنجاح\n\n📄 اسم الملف: {new_file_name}", reply_markup=create_run_keyboard(new_file_name))
    ...
    if message.document:
        ...
        if file_name.endswith('.py'):
            ...
            if old_file_name in file_storage:
                ...
                bot.send_message(message.chat.id, f"✅ تم استبدال الملف: {old_file_name}", reply_markup=create_run_keyboard(old_file_name))
