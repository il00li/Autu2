...
TOKEN = '8052936091:AAEqZMQJOz5Ixklb_DHocdKmUzzJHTkcuk4'
...
def show_main_menu(chat_id, user_id, new_message=False):
    ...
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} بحث جديد", callback_data="search"))
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} الإعدادات", callback_data="settings"))
...
def verify_subscription(call):
    ...
    if not_subscribed:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} تحقق من الاشتراك", callback_data="check_subscription"))
        try:
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
...
def show_content_types(call):
    ...
    if user_id not in premium_users:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} ترقية إلى المميز", callback_data="upgrade_premium"))
        markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} رجوع", callback_data="back_to_main"))
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} Photos", callback_data="type_photo"),
        InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} Illustrations", callback_data="type_illustration")
    )
    markup.add(
        InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} 3D Models", callback_data="type_3d"),
        InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} Videos", callback_data="type_video")
    )
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} All", callback_data="type_all"))
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} رجوع", callback_data="back_to_main"))
...
def request_search_term(call):
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} الغاء البحث", callback_data="cancel_search"))
...
def process_search_term(message, user_id):
    ...
    if not results or len(results) == 0:
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} بحث جديد", callback_data="search"))
        markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} الرئيسية", callback_data="back_to_main"))
...
def search_pixabay(query, content_type):
    ...
    if content_type == 'photo':
        ...
        params['image_type'] = 'photo'  # Pixabay doesn't have a specific 3D type
        params['q'] += ' 3d'  # so we add 3d to the query
...
def show_result(chat_id, user_id, message_id=None):
    ...
    if current_index > 0:
        nav_buttons.append(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} السابق", callback_data=f"nav_prev"))
    
    nav_buttons.append(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} تحميل", callback_data="download"))
    
    if current_index < len(results) - 1:
        nav_buttons.append(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} التالي", callback_data=f"nav_next"))
    
    markup.row(*nav_buttons)
    markup.row(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} جديد", callback_data="search"))
    markup.row(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} الرئيسية", callback_data="back_to_main"))
...
def show_settings(call):
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} إحصائيات", callback_data="user_stats"))
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} عن المطور", callback_data="about_dev"))
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} رابط الدعوة", callback_data="referral_link"))
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} ملحقات مجانية", url=f"https://t.me/{UPLOAD_CHANNEL[1:]}"))
...
👑 حالة العضوية: {'مميز' if user_id in premium_users else 'عادي'}
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton(f"{random.choice(GREEN_EMOJIS)} رجوع", callback_data="settings"))
