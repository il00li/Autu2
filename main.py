...
def generate_python_code(description):
    ...
    prompt = f"""
    IMPORTANT: Respond ONLY with pure Python code. No explanations, no comments, no markdown formatting.

    Create a complete, ready-to-run Telegram bot using the pyTelegramBotAPI library based on this description:
    {description}

    Requirements:
    - Use the telebot library (import telebot)
    - The code must be complete and error-free
    - Include basic functions like /start command and message handling
    ...
    - The bot should be ready to run immediately after creation

    Provide ONLY the Python code without any additional text.
    """
    ...
    try:
        ...
        if 'candidates' in result and len(result['candidates']) > 0:
            ...
            # إزالة أي نص غير بايثون (مثل "Here's your Python code:")
            lines = code.split('\n')
            python_lines = []
            in_code = False
...
أنا بот مجاني لرفع وتشغيل ملفات البايثون وإنشاء بوتات تلجرام تلقائياً.
...
• الملفات المحفوظة: {len(user_stats.get('files', []))}
...
"""
        ...
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"📂 الملفات المرفوعة ({len(user_files)}):\n{files_list}",
            reply_markup=create_more_options_keyboard()
        )
...
def process_bot_description(message):
    ...
    if code:
        ...
        # حفظ الملف في القناة
        if save_file_to_channel(code, file_name, user_id):
            ...
            bot.send_message(message.chat.id, "❌ حدث خطأ في حفظ الملف. يرجى المحاولة مرة أخرى.")
...
def handle_document(message):
    ...
    try:
        ...
        if save_file_to_channel(file_content, file_name, user_id):
            bot.send_message(chat_id, f"✅ تم رفع الملف بنجاح: {file_name}")
            logging.info(f"File uploaded: {file_name} by user {user_id}")
        else:
            bot.send_message(chat_id, "❌ حدث خطأ في حفظ الملف. يرجى المحاولة مرة أخرى.")
