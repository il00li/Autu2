...
def show_file_list(chat_id, action):
    if not file_storage:
    ...
    bot.send_message(chat_id, "📁 اختر ملف من القائمة:", reply_markup=keyboard)
...
def delete_file(file_name, chat_id):
    if file_name in file_storage:
        ...
        bot.send_message(chat_id, f"✅ تم حذف الملف: {file_name}")
    else:
...
def handle_callback(call):
    ...
    if call.data == "create_bot":
        ...
        show_file_list(call.message.chat.id, "confirm_delete")
    
    elif call.data == "replace_file":
        show_file_list(call.message.chat.id, "select_replace")
        ...
        bot.send_message(call.message.chat.id, "❌ تم إلغاء الإجراء")
