...
bot = telebot.TeleBot(TELEGRAM_BOT_TOKEN, state_storage=storage)
...
def handle_create_bot(call):
    ...
    bot.set_state(call.from_user.id, BotStates.DESCRIPTION, call.message.chat.id)
...
def handle_useful_options(call):
    """عرض خيارات مفيدة"""
    bot.answer_callback_query(call.id)
...
def handle_upload_file(call):
    """معالجة رفع ملف جاهز"""
    bot.answer_callback_query(call.id)
    bot.set_state(call.from_user.id, BotStates.WAITING_FILE, call.message.chat.id)
...
def handle_create_file(call):
    """بدء عملية إنشاء ملف"""
    bot.answer_callback_query(call.id)
    bot.set_state(call.from_user.id, BotStates.WAITING_CODE, call.message.chat.id)
...
def handle_code_input(message):
    ...
    bot.send_message(message.chat.id, "تم حفظ جزء الكود. استمر في إرسال الأجزاء أو ارسل /done عند الانتهاء")

@bot.message_handler(commands=['done'], state=BotStates.WAITING_CODE)
def handle_done_code(message):
    """إنهاء عملية إنشاء الملف"""
    user_id = message.from_user.id
    
    # إرسال رسالة التقدم
    progress_msg = bot.send_message(message.chat.id, "جاري إنشاء الملف وحفظه...")
    
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        code_parts = data.get('code_parts', [])
    
    if not code_parts:
        bot.send_message(message.chat.id, "لم يتم إرسال أي كود. يرجى المحاولة مرة أخرى.")
    ...
    try:
        # تحديث رسالة التقدم
        bot.edit_message_text("جاري رفع الملف إلى القناة...", message.chat.id, progress_msg.message_id)
        ...
        # تحديث رسالة التقدم
        bot.edit_message_text("جاري إرسال الملف النهائي...", message.chat.id, progress_msg.message_id)
        ...
        bot.send_message(message.chat.id, "✅ تم إنشاء الملف بنجاح!")
        
    except Exception as e:
        logger.error(f"Error uploading file to channel: {e}")
        bot.edit_message_text("❌ حدث خطأ أثناء حفظ الملف", message.chat.id, progress_msg.message_id)
...
def handle_file_upload(message):
    ...
    # إرسال رسالة التقدم
    progress_msg = bot.send_message(message.chat.id, "جاري معالجة الملف...")
    
    try:
        # تحديث رسالة التقدم
        bot.edit_message_text("جاري تحميل الملف...", message.chat.id, progress_msg.message_id)
        ...
        # تحديث رسالة التقدم
        bot.edit_message_text("جاري إكمال العملية...", message.chat.id, progress_msg.message_id)
        ...
        bot.edit_message_text("❌ حدث خطأ أثناء رفع الملف", message.chat.id, progress_msg.message_id)
...
def handle_select_replace(call):
    ...
    bot.set_state(call.from_user.id, BotStates.WAITING_NEW_CODE, call.message.chat.id)
...
def handle_new_code_input(message):
    """معالجة إدخال الكود الجديد"""
    if message.text == '/done':
        handle_done_new_code(message)
        return
...
@bot.message_handler(commands=['done'], state=BotStates.WAITING_NEW_CODE)
def handle_done_new_code(message):
    """إنهاء عملية استبدال الملف"""
    user_id = message.from_user.id
    
    # إرسال رسالة التقدم
    progress_msg = bot.send_message(message.chat.id, "جاري استبدال الملف...")
    ...
    if not code_parts or not filename:
        bot.send_message(message.chat.id, "لم يتم إرسال أي كود أو لم يتم تحديد ملف. يرجى المحاولة مرة أخرى")
        bot.delete_state(message.from_user.id, message.chat.id)
        return
    ...
    try:
        # تحديث رسالة التقدم
        bot.edit_message_text("جاري رفع الملف الجديد إلى القناة...", message.chat.id, progress_msg.message_id)
        ...
        # إرسال رسالة النجاح
        bot.delete_message(message.chat.id, progress_msg.message_id)
        bot.send_message(message.chat.id, "✅ تم استبدال الملف بنجاح!")
        
    except Exception as e:
        logger.error(f"Error replacing file: {e}")
        bot.edit_message_text("❌ حدث خطأ أثناء استبدال الملف", message.chat.id, progress_msg.message_id)
...
def handle_run_file(call):
    ...
    bot.set_state(call.from_user.id, BotStates.WAITING_RUN_MODE, call.message.chat.id)
...
@bot.callback_query_handler(func=lambda call: call.data in ['run_permanent', 'run_development'], state=BotStates.WAITING_RUN_MODE)
def handle_run_mode(call):
    ...
    # إرسال رسالة التقدم
    progress_msg = bot.send_message(call.message.chat.id, "جاري تحضير الملف للتشغيل...")
    ...
    try:
        # تحديث رسالة التقدم
        bot.edit_message_text("جاري تحميل الملف...", call.message.chat.id, progress_msg.message_id)
        ...
        if mode == 'run_permanent':
            run_file_permanent(call, local_filename, user_id, progress_msg)
        else:  # run_development
            run_file_development(call, local_filename, user_id, progress_msg)
...
def run_file_permanent(call, filename, user_id, progress_msg):
...
def run_file_development(call, filename, user_id, progress_msg):
...
def generate_bot(message):
    ...
    wait_msg = bot.send_message(message.chat.id, "جاري تحليل طلبك وإنشاء البوت...")
    ...
    try:
        ...
        bot.edit_message_text("جاري التواصل مع الذكاء الاصطناعي...", message.chat.id, wait_msg.message_id)
        ...
        # تحديث رسالة الانتظار
        bot.edit_message_text("جاري إعداد الملف النهائي...", message.chat.id, wait_msg.message_id)
        
        # إنشاء أزرار
        keyboard = types.InlineKeyboardMarkup()
        
        if bot_token:
            keyboard.add(types.InlineKeyboardButton("▶️ تشغيل البوت", callback_data='run_bot'))
        else:
            keyboard.add(types.InlineKeyboardButton("🔑 إضافة توكن وتشغيل", callback_data='add_token'))
        
        keyboard.add(types.InlineKeyboardButton("✏️ تعديل الكود", callback_data='edit_code'))
        ...
        bot.edit_message_text("❌ عذراً، حدث خطأ أثناء إنشاء البوت. يرجى المحاولة مرة أخرى.", message.chat.id, wait_msg.message_id)
        bot.delete_state(message.from_user.id, message.chat.id)

@bot.callback_query_handler(func=lambda call: call.data == 'edit_code')
def handle_edit_code(call):
    """بدء عملية تعديل الكود"""
    bot.answer_callback_query(call.id)
    bot.set_state(call.from_user.id, BotStates.EDITING, call.message.chat.id)
...
def handle_edit(message):
    ...
    # إظهار رسالة الانتظار
    wait_msg = bot.send_message(message.chat.id, "جاري معالجة التعديلات...")
    ...
    try:
        ...
        # إرسال رسالة النجاح
        bot.delete_message(message.chat.id, wait_msg.message_id)
        bot.send_message(message.chat.id, "✅ تم تعديل البوت بنجاح!")
        
        bot.delete_state(message.from_user.id, message.chat.id)
        
    except Exception as e:
        logger.error(f"Error editing bot: {e}")
        bot.edit_message_text("❌ عذراً، حدث خطأ أثناء تعديل البوت. يرجى المحاولة مرة أخرى.", message.chat.id, wait_msg.message_id)
        bot.delete_state(message.from_user.id, message.chat.id)
...
def handle_add_token(call):
    ...
    bot.set_state(call.from_user.id, BotStates.WAITING_TOKEN, call.message.chat.id)
...
@bot.message_handler(state=BotStates.WAITING_TOKEN, content_types=['text'])
def handle_token_input(message):
    ...
    if not re.match(r'\d{8,10}:[A-Za-z0-9_-]{35}', token):
        bot.send_message(chat_id, "❌ صيغة التوكن غير صحيحة. يرجى إرسال توكن صالح.")
        return
    ...
    bot.send_message(chat_id, "✅ تم حفظ التوكن. جاري تشغيل البوت...")
    
    # تشغيل البوت
    run_generated_bot_from_message(message)
    
    bot.delete_state(user_id, chat_id)

def run_generated_bot_from_message(message):
    ...
    if not token:
        bot.send_message(chat_id, "❌ لم يتم تحديد توكن البوت. يرجى إرسال التوكن أولاً.")
        return
    ...
    # تشغيل البوت
    run_bot_process_message(message, filename, user_id)

def run_bot_process_message(message, filename, user_id):
    """تشغيل عملية البوت من رسالة"""
    # تشغيل البوت
    run_msg = bot.send_message(message.chat.id, "جاري تشغيل البوت...")
    ...
    # إنشاء أزرار للتحكم
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton("⏹️ إيقاف البوت الآن", callback_data='stop_bot'))
...
def handle_run_bot(call):
    ...
    # تشغيل البوت
    run_bot_process_call(call, filename, user_id)

def run_bot_process_call(call, filename, user_id):
...
def echo_all(message):
    ...
    if not state:
        bot.send_message(message.chat.id, "لم أفهم طلبك. يرجى استخدام الأوامر المتاحة أو /start للبدء.")
    else:
        # إذا كان المستخدم في حالة معينة، إعلامه بكيفية الخروج
        bot.send_message(message.chat.id, "يمكنك استخدام /cancel لإلغاء العملية الحالية.")
