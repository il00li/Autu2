...
async def generate_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if bot_token:
            keyboard.append([InlineKeyboardButton(f"{get_random_emoji()} تشغيل البوت", callback_data='run_bot')])
        else:
            keyboard.append([InlineKeyboardButton(f"{get_random_emoji()} إضافة توكن وتشغيل", callback_data='add_token')])
        
        keyboard.append([InlineKeyboardButton(f"{get_random_emoji()} تعديل الكود", callback_data='edit_code')])
        keyboard.append([InlineKeyboardButton(f"{get_random_emoji()} عرض الكود", callback_data='show_code')])
        keyboard.append([InlineKeyboardButton(f"{get_random_emoji()} الرجوع", callback_data='back_to_main')])
...
async def handle_done_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(f"{emoji3} الرجوع", callback_data='useful_options')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_token_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("هذه الميزة قيد التطوير حالياً")
    return ConversationHandler.END
