...
def handle_done_code(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "حدث خطأ أثناء حفظ الملف")
...
def handle_file_upload(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "حدث خطأ أثناء رفع الملف")
...
def handle_done_new_code(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "حدث خطأ أثناء استبدال الملف")
...
def handle_run_mode(call):
    ...
    try:
        ...
        if mode == 'run_permanent':
            run_file_permanent(call, local_filename, user_id)
        else:  # run_development
            run_file_development(call, local_filename, user_id)
...
def run_file_permanent(call, filename, user_id):
...
def run_file_development(call, filename, user_id):
...
def generate_bot(message):
    ...
    - أضح توكن البوت في متغير BOT_TOKEN (استخدم القيمة: {bot_token or "YOUR_BOT_TOKEN_HERE"})
    ...
    try:
        ...
        bot.delete_message(message.chat.id, wait_msg.message_id)
        bot.send_message(message.chat.id, "عذراً، حدث خطأ أثناء إنشاء البوت. يرجى المحاولة مرة أخرى.")
        bot.delete_state(message.from_user.id, message.chat.id)
...
def handle_edit(message):
    ...
    try:
        ...
        bot.delete_message(message.chat.id, wait_msg.message_id)
        bot.send_message(message.chat.id, "عذراً، حدث خطأ أثناء تعديل البوت. يرجى المحاولة مرة أخرى.")
        bot.delete_state(message.from_user.id, message.chat.id)
...
def echo_all(message):
    ...
    bot.send_message(message.chat.id, "لم أفهم طلبك. يرجى استخدام الأوامر المتاحة.")
