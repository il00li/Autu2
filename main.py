...
def get_user_data(user_id):
    ...
    try:
        # البحث عن رسالة المستخدم في قناة التخزين
        messages = bot.send_message(STORAGE_CHANNEL_ID, f"🔍 البحث عن بيانات المستخدم {user_id}")
        
        # في الواقع، سنحتاج إلى طريقة أفضل للبحث عن رسائل المستخدم
        # لهذا مثال مبسط، في التطبيق الحقيقي قد تحتاج إلى حفظ معرفات الرسائل
        user_data = {
            'user_id': user_id,
...
# دالة لتحديث بيانات المستخدم في قناة التخزين
...
🔗 رابط الدعوة: {user_data['referral_link']}
    ...
    try:
        if user_data['message_id']:
            ...
            # إذا لم يكن هناك رسالة، إنشاء رسالة جديدة
            sent_message = bot.send_message(STORAGE_CHANNEL_ID, user_message)
            user_data['message_id'] = sent_message.message_id
    except Exception as e:
...
def run_bot_for_15_minutes(file_path, chat_id, file_name, user_id):
    try:
        ...
        # إذا كان هناك أخطاء، إعادة النقطة للمستخدم
        if has_errors and user_id in active_processes:
            user_data = get_user_data(user_id)
        ...
        # إعادة النقطة للمستخدم في حالة الخطأ
...
def process_bot_creation(message):
    ...
    if code:
        ...
        # إذا فشل إنشاء البوت، إعادة النقطة للمستخدم
...
def process_edit_request(message):
    ...
    if user_id in bot.user_data and "action" in bot.user_data[user_id] and bot.user_data[user_id]["action"] == "editing":
        ...
        if modified_code:
            ...
            # إذا فشل التعديل، إعادة النقطة للمستخدم
...
if __name__ == "__main__":
    ...
    load_file_list()
    print("🤖 البوت النهائي مع نظام النقاط يعمل الآن...")
