...
def extract_required_libraries(file_content):
    ...
    for line in lines:
        line = line.strip()
        # اكتشاف عبارات الاستيراد
        if line.startswith('import '):
            lib = line.split('import ')[1].split()[0].split('.')[0]
            if lib not in libraries and lib not in ['os', 'sys', 'json', 'time', 're', 'math', 'datetime']:
...
def run_python_file_with_libraries(file_path, user_id):
    try:
        ...
        # الحصول على المكتبات المثبتة للمستخدم
        installed_libs = user_libraries.get(str(user_id), [])
        
        # تثبيت المكتبات المطلوبة غير المثبتة
        libraries_to_install = [lib for lib in required_libs if lib not in installed_libs]
        
        if libraries_to_install:
            install_cmd = ["pip", "install"] + libraries_to_install
            ...
            user_libraries[str(user_id)].extend(libraries_to_install)
            save_data()
...
1. اختر رفع ملف لرفع ملف بايثون وتشغيله
...
"""
    ...
    if call.data == "main_menu":
        ...
        msg = bot.send_message(call.message.chat.id, f"{get_random_emoji()} أرسل لي ملف بايثون (.py) لرفعه وتشغيله")
