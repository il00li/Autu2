...
async def generate_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if bot_token:
            keyboard.append([InlineKeyboardButton("تشغيل البوت", callback_data='run_bot')])
        else:
            keyboard.append([InlineKeyboardButton("إضافة توكن وتشغيل", callback_data='add_token')])
        
        keyboard.append([InlineKeyboardButton("تعديل الكود", callback_data='edit_code')])
...
async def run_generated_bot(query, context):
    ...
    if not token:
        await query.edit_message_text("لم يتم العثور على توكن البوت. يرجى استخدام زر إضافة توكن وتشغيل أولاً.")
        return
...
async def run_bot_process(update, context, filename, user_id, is_query=False):
    """تشغيل عملية البوت"""
    # تشغيل البوت
    run_msg = "جاري تشغيل البوت..."
    if is_query:
        await update.edit_message_text(run_msg)
    else:
    ...
    if is_query:
        await update.edit_message_text(success_msg, reply_markup=reply_markup)
    else:
