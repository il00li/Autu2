...
def generate_execution_plan(description):
    ...
    2. احرص على أن يكون الكود في ملف واحد 
    3. صمم واجهة المستخدم والأزرار المناسبة
    4. حدد الوظائف الأساسية والإضافية
    5. ضع خطة للتنفيذ فورا
    6. قدم الخطة بدون اي كود (نص فقط) 
    ...
    try:
        ...
        if 'candidates' in result and len(result['candidates']) > 0:
            ...
            plan = re.sub(r'[*_`#\]', '', plan)  # إزالة علامات التنسيق
            plan = re.sub(r'\n\s*\n', '\n\n', plan)  # إزالة الأسطر الفارغة الزائدة
            return plan.strip()
        else:
...
# استخدام Gemini AI لإنشاء الكود بناء على الخطة
def generate_code_from_plan(plan):
    ...
    المتطلبات الأساسية:
    1. استخدم مكتبة pyTelegramBotAPI (telebot) بشكل أساسي إضافة إلى أي مكتبات أخرى يحتاجها المستخدم في الخطة المقدمة
    ...
    8. تأكد من أن جميع الدوال تُستدعى بالمعاملات الصحيحة
    9. تجنب ترك دوال بدون معاملات إذا كانت تحتاج إليها
    ...
    data = {
        ...
        "generationConfig": {
            ...
            "topK": 40,
            "topP": 0.95,
            "maxOutputTokens": 8192,  # زيادة الحد الأقصى للسماح بأكواد أطول
        }
    }
    
    try:
        response = requests.post(url, headers=headers, json=data, timeout=60)  # زيادة المهلة
        response.raise_for_status()
        result = response.json()
        
        logging.info(f"Gemini API response for code: {json.dumps(result, ensure_ascii=False)}")
        ...
        if 'candidates' in result and len(result['candidates']) > 0:
            ...
            # إذا كان الكود طويلاً جداً، نقوم بتقسيمه
            if len(code.split('\n')) > 4000:
                lines = code.split('\n')
                code = '\n'.join(lines[:4000])
                code += "\n\n# تم تقصير الكود بسبب تجاوز الحد الأقصى لعدد الأسطر (4000 سطر)"
...
def run_python_file_with_libraries(file_path, user_id):
    try:
        # الحصول على المكتبات المطلوبة للمستخدم
        libraries = user_libraries.get(str(user_id), [])
        
        # إذا كان هناك مكتبات مطلوبة، قم بتثبيتها أولاً
        if libraries:
            ...
            if install_stderr and "ERROR" in install_stderr:
                return None, f"❌ خطأ في تثبيت المكتبات:\n{install_stderr}"
        ...
        # انتظار التنفيذ لمدة أقصاها 30 ثانية
        try:
            stdout, stderr = process.communicate(timeout=30)
        except subprocess.TimeoutExpired:
            process.kill()
            stdout, stderr = process.communicate()
            return None, "❌ تجاوز وقت التنفيذ (30 ثانية)"
...
"""
        ...
        if code:
            ...
            if success:
                ...
                # خصم طلب بعد النجاح في إنشاء الملف
        ...
        msg = bot.send_message(call.message.chat.id, "📦 أرسل أسماء المكتبات التي تريد تثبيتها مفصولة بمسافات:\n\nمثال: requests numpy pandas")
...
def process_install_libraries(message):
    ...
    if not libraries:
        bot.send_message(message.chat.id, "❌ لم تقم بإرسال أي مكتبات")
        return
    ...
    bot.send_message(message.chat.id, f"✅ تم حفظ المكتبات بنجاح: {', '.join(libraries)}\n\nسيتم تثبيت هذه المكتبات تلقائياً عند تشغيل أي ملف")
