...
"""
        ...
        bot.send_message(call.message.chat.id, "اختر الملف الذي تريد حذفه", reply_markup=keyboard)
    
    elif call.data.startswith("confirm_delete_"):
        file_name = call.data[15:]
        bot.send_message(call.message.chat.id, f"هل أنت متأكد من حذف الملف: {file_name}", reply_markup=create_confirm_delete_keyboard(file_name))
...
def process_bot_creation(message):
    ...
    if code:
        ...
        try:
            ...
            bot.send_message(message.chat.id, f"لم أتمكن من إرسال الملف إلى القناة: {str(e)}")
    else:
        if not is_admin(message.from_user.id):
