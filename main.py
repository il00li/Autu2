...
def generate_python_code(description):
    ...
    prompt = f"""
    قم بإنشاء كود بايثون كامل لبناء بوت تلجرام بناء على الوصف التالي:
    {description}
    
    المتطلبات الصارمة:
    - يجب أن يكون الكود جاهز للتشغيل مباشرة
    - استخدم مكتبة pyTelegramBotAPI (telebot)
    - لا تضيف أي تعليقات أو شروحات في الكود
    - لا تستخدم علامات ```python أو ``` في البداية أو النهاية
    - تأكد من أن الكود خالٍ من الأخطاء
    - أضف وظائف أساسية مثل /start ومعالجة الرسائل
    - تأكد من أن الكود متكامل ولا ينقصه أي شيء
    - لا تترك أي أماكن فارغة يجب تعبئتها من قبل المستخدم
    
    يجب أن يبدو الكود احترافياً ونظيفاً بدون أي تعليقات.
    """
    ...
    try:
        ...
        if 'candidates' in result and len(result['candidates']) > 0:
            ...
            logging.error(f"Gemini API error: {result}")
            return None
    except Exception as e:
